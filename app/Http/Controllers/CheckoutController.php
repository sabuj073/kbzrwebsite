...
class CheckoutController extends Controller
{
    ...
    {
        $shipping_info = $this->store_shipping_info_new($request);
        // if guest checkout, create user
        if(auth()->user() == null){
            $guest_user = $this->createUser_new($shipping_info);
            if($guest_user == 0){
    ...
    public function createUser()
    {
        ...
        $data = User::where('phone','like','%'.$guest_shipping_info['phone'].'%')->first();
        if(!$data){
        // User Create
        $user = new User();
        ...
        $address->security_code = $guest_shipping_info['security_code'];
        $address->delivery_where = $guest_shipping_info['delivery_where'];
    ...
    public function createUser_new($guest_shipping_info)
    {
        $success = 1;
        $password = substr(hash('sha512', rand()), 0, 8);
        $isEmailVerificationEnabled = get_setting('email_verification');
    ...
    {   
        ...
        if($auth_user != null){
            ...
            if(get_setting('guest_checkout_activation') == 1){
                ...
                $shipping_info['security_code'] = $request->security_code;
                $shipping_info['delivery_where'] = $request->delivery_where;
                $shipping_info['postal_code'] = $request->postal_code;
                if(get_setting("country_code")){
     ...
     public function store_shipping_info_new($request)
    {   
        $auth_user = auth()->user();
        if($auth_user == null && get_setting('guest_checkout_activation') == 0){
            return redirect()->route('user.login');
